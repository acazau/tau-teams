...
function handleTeamEvent(event) {
  ...
  if (!card) {
    ...
    const roleLabel = role === 'lead' ? 'Lead' : 'Worker';
    const header = document.createElement('div');
    header.className = 'tool-card-header';
    header.innerHTML = `
      <div class="tool-header-left">
        <span class="tool-card-chevron expanded"><svg width="8" height="8" viewBox="0 0 8 8" fill="currentColor"><path d="M2 1l4 3-4 3z"/></svg></span>
        <span class="tool-name">${roleLabel}: ${escapeHtmlInline(agent || 'Unknown')}</span>
        <span class="tool-args-preview">${escapeHtmlInline(team || '')}</span>
      </div>
      <div class="tool-header-right">
        <div class="tool-status streaming">streaming</div>
      </div>
    `;
    ...
    body.className = 'tool-card-body expanded';
  ...
  if (update) {
    if (update.type === 'streaming' && update.text) {
      ...
      if (status) { status.className = 'tool-status complete'; status.textContent = 'complete'; }
      // Collapse on done
